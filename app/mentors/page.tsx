    ...
    <div className="space-y-8">
      ...
      {mentors.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">No mentors found matching your query</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Try broadening your search term or clearing filters.</p>
        </div>
      ) : (
            ...
            <Card key={mentor.id} className="p-6 space-y-4 hover:shadow-lg transition-all flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-slate-900 dark:bg-brand-600 text-white flex items-center justify-center font-bold text-xl shrink-0 overflow-hidden border-2 border-brand-500">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 dark:text-white text-base">{mentor.user.name}</h3>
                    <p className="text-xs text-brand-600 dark:text-brand-400 font-medium line-clamp-1">{mentor.title}</p>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-3 leading-relaxed">{mentor.bio}</p>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between mt-4">
                <div>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider block">Session Fee</span>
                  <span className="text-lg font-extrabold text-slate-900 dark:text-white">${mentor.hourlyRate}/hr</span>
                </div>
