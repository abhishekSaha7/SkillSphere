...
import { ArrowLeft, Plus, Trash2, CheckCircle2, HelpCircle, Save } from 'lucide-react';
...
export default function NewAssessmentPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.questionType === 'MCQ' || q.questionType === 'MULTIPLE_ANSWER') {
    ...
    try {
      ...
      const res = await fetch(`/api/courses/${params.id}/assessments`, {
        method: 'POST',
      ...
      toast.success('🎉 Assessment Created & Attached to Course!');
    ...
    <div className="space-y-6 max-w-4xl mx-auto py-4">
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Create Course Assessment</h1>
          <Badge variant="info">Quiz Builder</Badge>
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Design quizzes, add questions, set correct answers, and assign a passing score threshold.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
          ...
          <Button type="submit" isLoading={isSubmitting} className="font-bold px-6">
            <Save className="w-4 h-4 mr-2" /> Save & Attach Assessment
          </Button>
        </div>
      </form>
    </div>
