    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {certificates.length === 0 ? (
        <Card className="p-12 text-center text-slate-500 border-dashed">
          ...
          <h3 className="text-base font-semibold text-slate-700">No certificates earned yet</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {certificates.map((cert) => (
            <Card key={cert.id} className="border-2 border-brand-200/80 shadow-md relative overflow-hidden bg-gradient-to-br from-white via-brand-50/30 to-white">
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <Badge variant="success" className="gap-1">
                    <ShieldCheck className="w-3.5 h-3.5" /> Official Certificate
                  </Badge>
                  <span className="text-[10px] font-semibold text-slate-400">ID: {cert.certificateCode.substring(0, 8)}</span>
                </div>

                <div className="space-y-1">
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">SkillSphere Accreditation</p>
                  <h3 className="font-extrabold text-slate-900 text-lg leading-tight">{cert.course.title}</h3>
                  <p className="text-xs text-slate-600">Issued to <strong className="text-slate-900">{cert.user.name}</strong></p>
                </div>
